...
- **HTTP メソッド**: `GET`
- **認証**: 必須 (APIキーによる認証)
  - 以下のいずれかの方法で有効なAPIキーを指定する必要があります：
    1. HTTP ヘッダー: `x-api-key: <YOUR_API_KEY>`
    2. HTTP 認証ヘッダー: `Authorization: Bearer <YOUR_API_KEY>`
    3. クエリパラメータ: `?api_key=<YOUR_API_KEY>`
...
リクエストに有効なAPIキーが含まれていない、またはキーが間違っている場合に返されます。
```json
{
  "error": "Unauthorized",
...
```
または、サーバー側でキーがまだ設定されていない場合：
```json
{
  "error": "Unauthorized",
  "message": "API access key is not configured on the server."
}
```
...
外部ディスプレイ等で表示する場合の、最もシンプルな fetch 処理（ヘッダーでAPIキーを渡す例）の実装サンプルです。

```javascript
async function getActiveItems() {
  const url = 'https://[Your-Domain]/api/items?status_id=1&limit=10';
  const apiKey = 'YOUR_API_KEY'; // 管理者ダッシュボードで生成したキー
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'x-api-key': apiKey // または 'Authorization': `Bearer ${apiKey}`
      }
    });
